...
class NewsAppWidget : AppWidgetProvider() {
    ...
    private fun scheduleUpdate(context: Context) {
        val work = OneTimeWorkRequestBuilder<WidgetDataWorker>()
            .setInputData(workDataOf("widget_type" to "news"))
            .setConstraints(
                Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build()
            )
            .build()
        WorkManager.getInstance(context)
            .enqueueUniqueWork("news_widget_now", ExistingWorkPolicy.REPLACE, work)
    }
    ...
    companion object {
        fun displayFromCache(context: Context, manager: AppWidgetManager, id: Int) {
            ...
            val json = prefs.getString("news_data", null)

            if (json != null) {
                try {
                    val type = object : TypeToken<List<NewsItem>>() {}.type
                    val news: List<NewsItem> = Gson().fromJson(json, type)
                    val display = news.take(5).mapIndexed { i, item ->
                        "${i + 1}. ${item.title}\n   ${item.source}"
                    }.joinToString("\n\n")
                    views.setTextViewText(R.id.news_content, display)
                    views.setTextViewText(R.id.news_count, "${news.size} articles")
                } catch (_: Exception) {
                    views.setTextViewText(R.id.news_content, "Error loading news")
                }
            } else {
